...
def qwen3_quest_forward(
    ...
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    past_key_value: Optional[Union[DynamicCache, Tuple[torch.Tensor]]] = None,
    output_attentions: bool = False,
    use_cache: bool = False,
    **kwargs,
) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
    ...
    # 1. Pre-fill 阶段或前两层：直接走原始 Full Attention (flash_forward)
    # Qwen3 的前几层对于捕捉全局上下文至关重要
    if q_len > 1 or self.layer_id < 2:
        return self.flash_forward(
            hidden_states,
            attention_mask,
    ...
    # 2. Q/K/V 投影 (注意：Qwen3 的投影层通常包含 Bias)
    ...
    # 3. 计算旋转位置编码 (RoPE)
    # Qwen3 的 rotary_emb 调用方式通常与 Llama 一致，但需确保设备匹配
    kv_seq_len = key_states.shape[-2]
    if past_key_value is not None:
        if isinstance(past_key_value, DynamicCache):
    ...
    # 4. 更新 KV Cache
    if isinstance(past_key_value, DynamicCache):
        key_states, value_states = past_key_value.update(key_states, value_states, self.layer_id, kwargs)
    else:
        if past_key_value is not None:
            key_states = torch.cat([past_key_value[0], key_states], dim=2)
            value_states = torch.cat([past_key_value[1], value_states], dim=2)
    ...
    # 估算重要性 (Heavy Hitter Selection)
    # 使用 Query 和 Key 的符号/幅值进行量化估算
    sign = (query_states > 0).float() * 2 - 1
    max_key = key_states * sign
    ...
    # 对 Key 进行 Chunking 和 Max Pooling
    max_key_padded = torch.cat([
        max_key,
        torch.ones((bsz, self.num_heads, padding_length, self.head_dim), 
                   device=max_key.device, dtype=max_key.dtype) * torch.finfo(max_key.dtype).min
    ], dim=-2)

    chunk_max_key = max_key_padded.reshape(
        bsz, self.num_heads, max_key_padded.shape[2] // self.chunk_size, self.chunk_size, self.head_dim
    ).amax(dim=-2)

    # 重新广播并计算量化后的权重
    chunk_max_key_up = chunk_max_key.unsqueeze(-2).repeat(1, 1, 1, self.chunk_size, 1).reshape(
        bsz, self.num_heads, -1, self.head_dim
    )[:, :, :seq_length, :]

    quantized_weight = torch.matmul(positive_query.float(), chunk_max_key_up.transpose(2, 3))
    ...
    # 确保不观测未来的 Token (Causal Mask)
    current_pos = position_ids[0][0].item() if position_ids is not None else kv_seq_len - 1
    ...
    for name, module in reversed(model._modules.items()):
        ...
        if isinstance(module, (LlamaAttention, MistralAttention, Qwen3Attention)):
            ...
            model._modules[name].forward = types.MethodType(
                qwen3_quest_forward, model._modules[name]
            ) #在这里改变前向函数
            print(f"qwen---Enabled Quest Attention in layer {layer_id}")
