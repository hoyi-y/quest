# Based on HuggingFace Qwen3 Model: transformers.models.qwen3.modeling_qwen3
# transformers>=4.45

"""PyTorch Qwen3 model with QuestAttention."""
import math
...
class QuestQwen3Attention(nn.Module):
    """Quest attention for Qwen3 with q_norm/k_norm and GQA repeat."""
    ...
    def _repeat_kv(self, kv: torch.Tensor) -> torch.Tensor:
        if kv.size(1) == self.num_heads:
            return kv
        if kv.size(1) != self.num_key_value_heads:
            raise ValueError("Unexpected KV head count for GQA repeat.")
        repeat = self.num_heads // self.num_key_value_heads
        if repeat * self.num_key_value_heads != self.num_heads:
            raise ValueError("num_heads must be divisible by num_key_value_heads for GQA.")
        return kv.repeat_interleave(repeat, dim=1)

    def _rms_norm_fp32(self, x: torch.Tensor, weight: torch.Tensor, eps: float) -> torch.Tensor:
        # Align with HF Qwen3: do RMSNorm in fp32 for numerical stability.
...
class Qwen3DecoderLayer(nn.Module):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = QuestQwen3Attention(config=config, layer_idx=layer_idx)
